...
namespace IntegratedAuthoringToolTutorial
{
    class Program
    {
        ...
        public static void TEST()
        {
            ...
            var storage = AssetStorage.FromJson(File.ReadAllText("../../../../Examples/IAT-Tutorial/Scenarios/TestKbStorage.json"));

            //Loading the asset
            var iat = IntegratedAuthoringToolAsset.FromJson(File.ReadAllText("../../../../Examples/IAT-Tutorial/Scenarios/TestScenario.json"), storage);
            var characters = iat.Characters.ToList();
            ...
            characters.ForEach(c => {
                Console.WriteLine($"Character name {c.CharacterName}"); c.GetAllBeliefs().ToList().ForEach(b =>
                Console.WriteLine($"Character Belief : {b.Name} = {b.Value}"));
            });

            //var beliefs = rpc.m_kb.GetAllBeliefs();
            characters.FirstOrDefault().m_kb.Reason();
            characters.LastOrDefault().m_kb.Reason();
            ...
            //    Console.WriteLine("Character Beliefs:" + b.Name + "= " + b.Value);
            characters.ForEach(c => {
                Console.WriteLine($"Character name {c.BodyName}"); c.GetAllBeliefs().ToList().ForEach(b =>
                Console.WriteLine($"Character Belief : {b.Name} = {b.Value}"));
            });

            var Charlie = characters.FirstOrDefault();
            var CharlieName = Charlie.CharacterName.ToString();

            var Player = characters.FirstOrDefault();
            var PlayerName = IATConsts.PLAYER;
            ...
            while (currentState != IATConsts.TERMINAL_DIALOGUE_STATE)
            {
                ...
                var actionName = iat.BuildSpeakActionName(chosenDialog.Id);
                var speakEvt = EventHelper.ActionEnd(PlayerName, actionName.ToString(), CharlieName);

                currentState = chosenDialog.NextState;
                var dialogStateChangeEvt = EventHelper.PropertyChange(string.Format(IATConsts.DIALOGUE_STATE_PROPERTY, PlayerName), chosenDialog.NextState, CharlieName);

                Charlie.Perceive(speakEvt);
                Charlie.Perceive(dialogStateChangeEvt);
                var characterActions = Charlie.Decide();
                ...
                if (characterAction.Key.ToString() == IATConsts.DIALOG_ACTION_KEY)
                {
                    ...
                    var ThereAreMore = iat.GetDialogueActions(
                        characterAction.Parameters[0],
                        characterAction.Parameters[1],
                        characterAction.Parameters[2],
                    ...
                    Console.WriteLine("\n" + CharlieName + ": " + dialog.Utterance + "\n");
                    currentState = characterAction.Parameters[1].ToString();
                    Console.WriteLine(" Mood of Charlie : " + Charlie.Mood);

                }
                else
                {
                    Console.WriteLine("\n" + CharlieName + ": " + characterAction.Name + "\n");
                }
            }
